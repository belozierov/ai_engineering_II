...
struct StdioToolHostTests {
	...
	private func withHost<T>(
		...
		body: (Client, StdioToolHost) async throws -> T) async throws -> T {
		let host = try StdioToolHost(name: "test", version: "1.0.0", tools: tools, listGate: listGate)
		...
		do {
			let result = try await body(client, host)
	...
	private func withHost<T>(
		...
		try await withHost(tools: tools) { client, _ in try await body(client) }
	}
	...
	@Test
	func gateFlipsBetweenEmptyAndFullLists() async throws {
		let gate = GateState()

		try await withHost(tools: [EchoTool()], listGate: { await gate.isOpen }) { client, _ in
			let (closed, _) = try await client.listTools()
			#expect(closed.isEmpty)

			await gate.open()

			let (opened, _) = try await client.listTools()
			#expect(opened.map(\.name) == ["echo"])
		}
	}

	// The gate hides declarations, never dispatch: a client acting on a stale list still reaches
	// the tool, whose own refusal can explain itself — "unknown tool" cannot.
	@Test
	func gatedOffToolsStayCallable() async throws {
		try await withHost(tools: [EchoTool()], listGate: { false }) { client, _ in
	...
	@Test
	func notifyListChangedReachesTheClient() async throws {
		try await withHost(tools: [EchoTool()], listGate: { true }) { client, host in
			let (signals, continuation) = AsyncStream.makeStream(of: Void.self)
