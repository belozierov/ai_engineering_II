...
extension Invocation {
	...
	public struct MCPConfig: Sendable {
		...
		public struct Server: Sendable {
			...
			public init(name: String, executable: URL, arguments: [String] = [], environment: [String: String] = [:]) {
				self.name = name
		...
		func makeJSON() throws -> String {
			...
			for server in servers {
				...
				mcpServers[server.name] = Payload.Server(command: server.executable.path(), args: server.arguments, env: server.environment)
			}
...
extension Invocation.MCPConfig {
	...
	public init(proxy: Claude.ToolProxyCommand, port: UInt16) {
		self.init(servers: [
			Server(
